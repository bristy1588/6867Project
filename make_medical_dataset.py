...
TEST_PERCENT = .2
IMAGE_SIZE = 50
NUM_CHANNELS = 1 #RGB channels
PIXEL_DEPTH = 255.0
NUM_DISEASES = 15
PARTITION_TEST = False

def load_disease(disease):
  '''Load the images for a single disease.'''
  print("Loading images for disease", disease)
  folder = DATA_PATH + 'diseases/' + disease + '/'
  ...
  print('Medical dataset size:', dataset.shape)
...
def make_dataset_arrays(num_rows=15000):
  data = np.ndarray((num_rows, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS), dtype=np.float32)
  labels = np.ndarray((num_rows, NUM_DISEASES), dtype=np.int32)
  return data, labels
...
def make_basic_datasets():
  disease_path = DATA_PATH + 'diseases'
  diseases = [x for x in os.listdir(disease_path) if x != '.DS_Store']
  assert len(diseases) == NUM_DISEASES
  ...
  for label,disease in enumerate(diseases):
    # create a one-hot encoding of this disease's label
    disease_label = (np.arange(NUM_DISEASES) == label).astype(np.float32)
    disease_data = load_disease(disease)
    disease_data = scale_pixel_values(disease_data)
    num_xrays = len(disease_data)

    # randomly shuffle the data to ensure random validation and test sets
    np.random.shuffle(disease_data)

    nv = int(num_xrays * VALIDATION_PERCENT)

    # partition validation data
    disease_val = disease_data[0:nv, :, :, :]
    val_data[num_val:num_val+nv, :, :, :] = disease_val
    val_labels[num_val:num_val+nv, :] = disease_label
    num_val += nv

    # partition test data
    if PARTITION_TEST:
      nt = int(num_xrays * TEST_PERCENT)
      disease_test = disease_data[nv:nv+nt, :, :, :]
      test_data[num_test:num_test+nt, :, :, :] = disease_test
      test_labels[num_test:num_test+nt, :] = disease_label
      num_test += nt
    else:
      nt = 0

    # patition train data
    disease_train = disease_data[nv+nt:, :, :, :]
    ntr = len(disease_train)
    print("--- ntr and num_train is: ", ntr, num_train)
    print("###disease_train shape: ", disease_train.shape)
    train_data[num_train:num_train+ntr, :, :, :] = disease_train
    train_labels[num_train:num_train+ntr, :] = disease_label
    num_train += ntr
  ...
  # shuffle the data to distribute samples from diseases randomly
  ...
  pickle_file = 'medical_data.pickle'
  save = {
...
def augment_training_set():
  ...
  with open(DATA_PATH + 'medical_data.pickle', 'rb') as f:
...
def make_invariance_sets():
  ...
  save_pickle_file('medical_data.pickle', save)
...
if __name__ == '__main__':
  print("Making medical dataset and saving it to:", DATA_PATH)
